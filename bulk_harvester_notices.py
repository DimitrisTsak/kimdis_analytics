...
BASE_URL = "https://cerpp.eprocurement.gov.gr/khmdhs-opendata/notice"
...
def harvest_notices(start_page=0, pages_to_collect=50):
    headers = {
    ...
    print(f"Harvesting /notice starting at page {start_page} ({pages_to_collect} pages)...")
    ...
    while current_page < (start_page + pages_to_collect):
        ...
        try:
            ...
            for t in tenders:
                ...
                title = t.get("title")
                # /notice uses typeOfProcedure (not procedureType)
                proc_type = (t.get("typeOfProcedure") or {}).get("value")
                ...
                # Prefer signedDate for year, fall back to submissionDate
            ...
            print(f"  Page {current_page}: {len(batch_data)} notices committed.")
    ...
    conn.close()
    print(f"\nNotice harvest complete. Total: {total_records} records.")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Harvest /notice records into the analytics DB")
    ...
    harvest_notices(start_page=args.start_page, pages_to_collect=args.pages)
